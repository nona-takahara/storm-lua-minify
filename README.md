...
畳み込みの結果、Luaの出力上の意味は変わりません。ただし整数と浮動小数点数の区別は保たれます。`3/1`のような式は`3`ではなく`3.0`に畳み込まれます（Luaの`/`は常に浮動小数点数を返すため）。

安全性を優先し、次の操作は意図的に対象外にしています。

- `#`（長さ演算子）: 文字列のバイト長はエスケープ表現に依存するため
- 文字列を数値として使う算術（`"10" + 1`）: 表記の再現を保証できないため
- 数値を含む連結（`1 .. "x"`）: `1`と`1.0`で連結後の表記が変わるため
- 文字列同士の大小比較（`"a" < "b"`）: 照合順の前提を持ち込まないため
- ゼロ除算・ゼロ剰余（整数の`//`・`%`）と、結果が非有限になる浮動小数点演算: Luaの実行時エラー・特殊値をこのツールが勝手に解決してはならないため
- エスケープや長括弧（`[[...]]`）を含む文字列リテラル: バイト列としての表記を保持したまま安全に再結合できないため
- 大きすぎて64bit整数に収まらない10進整数リテラル: Luaではこの場合float扱いになるため、扱いを分けずに見送ります
