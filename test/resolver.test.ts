...
describe("lexical name resolution", () => {
  ...
  `);
    ...
    const innerDecl = (doStatement.body[0] as Parser.LocalStatement)
      .variables[0];
    ...
    const outerPrintArg = (
      (chunk.body[2] as Parser.CallStatement)
        .expression as Parser.CallExpression
    ).arguments[0] as Parser.Identifier;
  ...
  `);
    ...
    const multiplyExpr = returnStatement
      .arguments[0] as Parser.BinaryExpression;
  ...
  `);
    ...
    // The same spelling outside the loop is an unrelated global reference.
    const afterArg = (
      (chunk.body[1] as Parser.CallStatement)
        .expression as Parser.CallExpression
    ).arguments[0] as Parser.Identifier;
  ...
  `);
    ...
    // `screen` is undeclared, so all three occurrences are global references.
    const screenBinding = result.globals.get("screen");
    assert.ok(screenBinding);
    assert.equal(screenBinding.references.length, 3);

    // Field names and table keys are not variable references.
  ...
  test("GlobalBinding.writes records only assignment-target occurrences", () => {
    const chunk = parse(`
    print(readOnly)
    writable = 1
    writable = writable + 1
  ...
  `);
    ...
    // Two assignment targets and one read make three references.
    assert.equal(writableBinding.references.length, 3);
    assert.equal(writableBinding.writes.length, 2);

    // A global function declaration also counts as an assignment target.
  ...
  test("isGlobalReference distinguishes globals from same-spelling fields", () => {
    const chunk = parse(`
    counter = 1
    local t = {}
    t.counter = 2
